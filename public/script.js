...
class SnakeAI {
    ...
    getState(snake, food) {
      ...
      // Direção da comida em relação à cabeça da cobra (esquerda, direita, acima, abaixo)
  ...
  // Função para desenhar as cobras e a comida
  ...
  function drawFood(game) {
    ctx.fillStyle = 'red';
    ctx.fillRect(game.food.x, game.food.y, gridSize, gridSize);
  }
  ...
  function gameLoop() {
    ...
    snakes.forEach((game, index) => {
      ...
      drawSnake(game);  // Desenhar a cobra
      drawFood(game);   // Desenhar a comida
    });
